...
const CreateEmployee = () => {
    ...
    useEffect(() => {
        document.title = "Create Employee | EMS";
    },[]);
  ...
  return (
    ...
    <aside className={`fixed left-0 top-0 h-screen w-64 bg-white shadow-md p-6 overflow-y-auto z-50 transform transition-transform duration-300 ${sidebarOpen?"translate-x-0":"-translate-x-full"} lg:translate-x-0 lg:block`}>
      ...
      <main className='flex-1 md:p-8 lg:ml-64 p-4 overflow-x-auto'>
        ...
        <p className='text-md font-medium text-black mb-8'>You can create employees here.</p>
        <div className='bg-white shadow-lg rounded-lg p-8'>
            <h1 className='text-2xl font-bold mb-6'>Create Employees</h1>
            <form onSubmit={handleSubmit} className='space-y-6'>
                ...
                <div>
                    <button type="submit" disabled={loading} className='w-full bg-green-700 px-4 py-2 text-white hover:bg-green-600 cursor-pointer'>{loading?"Creating the employee":"Create Employee"}</button>
                </div>
            </form>
        </div>
      </main>
