    ...
    <div className='bg-blue-100 flex min-h-screen'>
      ...
      <aside className={`fixed left-0 top-0 h-screen w-64 bg-white shadow-md p-6 overflow-y-auto z-50 transform transition-transform duration-300 ${sidebarOpen?"translate-x-0":"-translate-x-full"} lg:translate-x-0 lg:block`}>
        ...
        <nav className='space-y-4'>
          <Link href="/dashboard/employee" onClick={()=>setSidebarOpen(false)} className='block font-bold p-2 rounded hover:bg-green-200'>Dashboard</Link>
      ...
      <main className='flex-1 md:p-8 lg:ml-64 p-4 overflow-x-auto'>
        ...
        <div className='bg-white mt-6 p-3 md:p-8 shadow rounded-lg '>
          ...
          <div className='mt-6'>
            ...
            <p className='text-xl font-medium'>{profile?.department}</p>
          </div>
          <div className='mt-6'>
            <p className='text-xl font-semibold'>Designation:</p>
            <p className='text-xl font-medium'>{profile?.designation}</p>
          </div>
        </div>
      </main>
      {toast && (
